...
export default function ClaimReward({
  ...
  for (let i = 0; i < numOfWinners; i++) {
    winners.push(<GetWinner i={i} lotContractAddress={lotContractAddress} />)
  }
  return (
    <div className="flex flex-col justify-center">
      <h1 className="text-center text-2xl my-2 py-2">Claim Rewards</h1>
      <div>
        <h2 className="text-center text-xl my-2 py-2">Winner Addresses</h2>
        <Web3Button
...
function GetWinner({ i, lotContractAddress }: GetWinnerProps) {
  ...
  const { data: winnerIndex } = useContractRead(lotcontract, "winnerIndexes", [
    i.toString(),
  ])
